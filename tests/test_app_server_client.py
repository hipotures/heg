...
class AppServerClientTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_persisted_structured_turn_and_usage(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIsNotNone(result.first_item_latency_seconds)
            self.assertIsNotNone(result.final_answer_latency_seconds)
            self.assertGreaterEqual(result.first_item_latency_seconds, 0)
            self.assertGreaterEqual(result.final_answer_latency_seconds, 0)
