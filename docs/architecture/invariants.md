...
| Exact facts/current executable IDs are non-droppable | memory projection, integrity-checked recovery registry, atomic checkpoint-batch pinning, irreducible-floor recovery and complete-request compaction before final Director limit | missing-checkpoint, self-eviction, forced-compaction, floor-search, request-budget and SnapshotBuilder ordering tests | unavailable references remain historical; an executable batch never evicts itself; `scientific_state_overflow` |
