...
.mono,.id{font-family:var(--mono);font-size:.86em}.id{overflow-wrap:anywhere}
...
@media(max-width:900px){.split,.grid.two{grid-template-columns:1fr}.header-inner{align-items:flex-start}.brand{min-width:0}.plan-callout{grid-template-columns:1fr}.hide-tablet{display:none}}
@media(max-width:700px){
  .site-header{position:static}.header-inner{padding:.8rem 1rem;display:grid;grid-template-columns:1fr auto}.brand{grid-column:1}
  nav{grid-column:1/-1;margin:0;overflow-x:auto;flex-wrap:nowrap;padding-bottom:.2rem}nav a{white-space:nowrap}.theme-toggle{grid-column:2;grid-row:1;min-width:0}
...
const idHtml=v=>v?`<span class="id" title="${esc(v)}">${esc(shortId(v))}</span>`:'—';
const field=(k,v)=>v===null||v===undefined||v===''?'':`<div class="semantic-field"><dt>${esc(label(k))}</dt><dd>${typeof v==='string'&&(/(^|_)(id|sha256|hash)$/.test(k)||v.length>36)?idHtml(v):esc(v)}</dd></div>`;
...
<div class="grid two"><article class="answer"><span class="eyebrow">Answer A</span><div id="a"></div></article><article class="answer"><span class="eyebrow">Answer B</span><div id="b"></div></article></div>
...
async function load(){{try{{const d=await api(`/api/comparisons/${{sid}}?blind=1`);blindSeed=d.blind_order_seed;pair=d.turns.filter(t=>t.lifecycle_status==='completed'&&t.schema_valid).slice(0,2);if(pair.length<2){{a.innerHTML=b.innerHTML='';document.getElementById('blind-controls').hidden=true;throw new Error('Two completed schema-valid turns are required for ordinary blind quality review.')}}a.innerHTML=decisionCard(pair[0].normalized_decision_json,{{}});b.innerHTML=decisionCard(pair[1].normalized_decision_json,{{}})}}catch(e){{notice.textContent=e.message}}}}
...
function profileCard(p){{return `<article class="suite-card"><div class="card-head"><div><div class="chips">${{badge(p.enabled?'enabled':'disabled')}}${{badge(p.reasoning_effort)}}</div><h3>${{esc(p.display_name)}}</h3><div class="meta">${{esc(p.model)}} · effective ${{esc(p.effective_from)}}</div></div><div class="metric emphasis"><small>Relative multiplier</small><strong>${{Number(p.relative_cost_multiplier).toFixed(3)}}×</strong></div></div><div class="grid">${{field('API input / million',p.api_input_per_million===null?'Not configured':`${{p.api_input_per_million}} ${{p.currency||''}}`)}}${{field('API cached input / million',p.api_cached_input_per_million===null?'Not configured':`${{p.api_cached_input_per_million}} ${{p.currency||''}}`)}}${{field('API output / million',p.api_output_per_million===null?'Not configured':`${{p.api_output_per_million}} ${{p.currency||''}}`)}}${{field('Source',p.source_label)}}${{field('Profile ID',p.profile_id)}}</div>${{technical('Technical profile record',p)}}</article>`}}
