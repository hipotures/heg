...
function workerCard(d){{const w=d.worker||{{}},lease=w.lease||{{}},request=w.stop_request||{{}},samples=w.resource_samples||[];const heartbeat=lease.heartbeat_at?Math.max(0,(Date.now()-Date.parse(lease.heartbeat_at))/1000):null;const sample=(category,kind)=>samples.filter(x=>x.category===category&&x.sample_kind===kind).at(-1);const scratch=sample('runtime_scratch','latest')||sample('runtime_scratch','terminal')||{{}},preserved=sample('preserved_artifacts','latest')||sample('preserved_artifacts','terminal')||{{}};const largest=scratch.largest_contributor_relative_path||preserved.largest_contributor_relative_path||d.suite.resource_largest_contributor||'Unavailable';const legacyFailure=!d.suite.resource_exceeded_category&&String(d.suite.failure_reason||'').includes('artifact directory limit exceeded');const category=d.suite.resource_exceeded_category||(legacyFailure?'legacy ambiguous artifact directory':null);const failure=category?`<div class="error-state" style="margin-top:.8rem"><strong>Infrastructure resource failure</strong><div class="grid" style="margin-top:.6rem">${{field('Category exceeded',category)}}${{field('Limit',fmt(d.suite.resource_exceeded_limit_bytes||(legacyFailure?d.suite.maximum_artifact_directory_bytes:null)))}}${{field('Peak',fmt(d.suite.resource_peak_bytes))}}${{field('Largest contributor',d.suite.resource_largest_contributor||'Transient contributor was not persisted')}}${{field('Stage',d.suite.resource_enforcement_stage||(legacyFailure?'post-arm artifact check':'Unavailable'))}}${{field('Active turn completed',d.suite.resource_active_turn_completed===null?(legacyFailure?'Yes':'Unavailable'):(d.suite.resource_active_turn_completed?'Yes':'No'))}}${{field('Later arms blocked',d.arms.some(a=>a.display_lifecycle_state==='blocked / not started'||a.lifecycle_state==='blocked')?'Yes':'No')}}${{field('Comparison complete','No')}}</div></div>`:'';return `<div class="section-heading"><div><h2>Comparison worker progress</h2><p>Bounded live status from the persisted lease, transitions, and resource peaks.</p></div>${{badge(lease.released_at?'released':lease.lease_id?'active':'inactive')}}</div><div class="grid">${{field('Completed arms',`${{w.completed_arms||0}} / ${{w.planned_arms||d.arms.length}}`)}}${{field('Worker PID',lease.pid??'Inactive')}}${{field('Heartbeat age',heartbeat===null?'Unavailable':fmtSeconds(heartbeat))}}${{field('Lease released',lease.released_at?'Yes':'No')}}${{field('Stop state',request.state||'Not requested')}}${{field('Inference starts',`${{d.suite.consumed_inference_starts}} / ${{d.suite.maximum_inference_starts}}`)}}${{field('Scratch current / peak',scratch.current_apparent_bytes===undefined?'Unavailable':`${{fmt(scratch.current_apparent_bytes)}} / ${{fmt(scratch.peak_apparent_bytes)}}`)}}${{field('Preserved current / peak',preserved.current_apparent_bytes===undefined?'Unavailable':`${{fmt(preserved.current_apparent_bytes)}} / ${{fmt(preserved.peak_apparent_bytes)}}`)}}${{field('Largest runtime contributor',largest)}}${{field('Quota status',category?'Exceeded':'Within configured limits')}}${{field('Cleanup status',d.suite.resource_cleanup_status||'Not recorded')}}${{field('Total server tokens',fmt(w.total_server_tokens||0))}}</div>${{failure}}`}}
